...
class DyNFFM_concat_dot(nn.Module):
    def __init__(self, fnames, max_idxs, embedding_size=4, dropout_rate=None, batch_norm=True, use_cuda=True):
        ...
        super(DyNFFM_concat_dot, self).__init__()
        self.fnames = fnames
        ...
        # embedding lr layer
        # bi-interation + embedding concat + bi_dot
        self.embLr_input_dim = self.n_bi * self.embedding_size + self.n_fnames * self.n_fnames * self.embedding_size + self.n_bi 
    ...
    def forward(self, static_ids, dynamic_ids, dynamic_lengths, conversion_rates):
        ...
        # batch dot layer
        dot_embeddings = torch.sum(bi_embeddings.view(batch_size, self.n_bi, self.embedding_size), -1)

        # embedding lr layer
        # B*F1*E + B*F2*E -> B*[F1+F2]*E -> B*[F*E]
        lr_out = torch.cat([bi_embeddings.view(batch_size, -1), all_embeddings.view(batch_size, -1), dot_embeddings.view(batch_size, -1)], -1)
