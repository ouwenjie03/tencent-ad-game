...
class DyNFFM_concat_triple(nn.Module):
    def __init__(self, fnames, max_idxs, embedding_size=4, dropout_rate=None, batch_norm=True, use_cuda=True):
        ...
        super(DyNFFM_concat_triple, self).__init__()
        self.fnames = fnames
        ...
        for i in range(self.n_fnames):
            ...
            for j in range(self.n_fnames):
                ...
                #  if i >= j or filter_same_features(all_fnames[i], all_fnames[j]):
                    ...
                    tmp_triple_mask = [1] * self.n_fnames
                    tmp_triple_mask[i] = 0
                    tmp_triple_mask[j] = 0
                    self.triple_masks.append(tmp_triple_mask)
            self.mask.append(tmp)
        self.mask = torch.autograd.Variable(torch.ByteTensor(self.mask))
        # N_BI*N
        self.triple_masks = torch.autograd.Variable(torch.ByteTensor(self.triple_masks))

        # embedding lr layer, 3 layers
        # bi-interation + one-order-embedding + triple-interation
        #  self.embLr_input_dim = self.n_bi * self.embedding_size + self.n_fnames * (self.n_fnames + 1) * self.embedding_size + self.n_bi * (self.n_fnames - 2) 
        self.embLr_input_dim = self.n_bi * self.embedding_size + self.n_fnames * (self.n_fnames + 1) * self.embedding_size + self.n_bi * self.n_fnames 
        self.hidden_sizes = [self.embLr_input_dim, 512, 256, 128]
        ...
        if self.use_cuda:
            ...
            self.triple_masks = self.triple_masks.cuda()
    ...
    def forward(self, static_ids, dynamic_ids, dynamic_lengths, conversion_rates):
        ...
        bi_embeddings = bi_embeddings.view(batch_size, self.n_bi, -1)

        # one-order embedding
        one_dynamic_embeddings = self.one_dyEmb(dynamic_ids, dynamic_lengths)
        one_static_embeddings = self.one_stEmb(static_ids)
        one_all_embeddings = torch.cat([one_static_embeddings, one_dynamic_embeddings], 1)

        # triple combine embeddings
        '''
        # too slow
        tri_embeddings = []
        for i in range(self.n_bi):
            tmp_triple_mask = self.triple_masks[i].view(1, self.n_fnames, 1).expand_as(one_all_embeddings)
            tmp_triple_embeddings = torch.masked_select(one_all_embeddings, tmp_triple_mask).view(batch_size, (self.n_fnames-2), -1)
            # B*(N-2)*E
            tmp_bi_embeddings = bi_embeddings[:, i, :].contiguous().view(batch_size, 1, -1)
            tmp_bi_embeddings = tmp_bi_embeddings.expand_as(tmp_triple_embeddings)
            # triple dot    B*1*(N-2)
            tri_embeddings.append(torch.sum(tmp_bi_embeddings * tmp_triple_embeddings, -1).view(batch_size, 1, (self.n_fnames-2)))
        # B * N_BI * (N-2)
        tri_embeddings = torch.cat(tri_embeddings, 1)
        '''

        '''
        # out of memory because of [expand and masked_select]
        tmp_all_embeddings = one_all_embeddings.view(batch_size, 1, self.n_fnames, self.embedding_size).expand(batch_size, self.n_bi, self.n_fnames, self.embedding_size)
        tmp_triple_mask = self.triple_masks.view(1, self.n_bi, self.n_fnames, 1).expand_as(tmp_all_embeddings)
        tmp_triple_embeddings = torch.masked_select(tmp_all_embeddings, tmp_triple_mask).view(batch_size, self.n_bi, (self.n_fnames-2), -1)
        tmp_bi_embeddings = bi_embeddings.view(batch_size, self.n_bi, 1, -1).expand_as(tmp_triple_embeddings)
        # B * N_BI * (N-2)
        tri_embeddings = torch.sum(tmp_bi_embeddings * tmp_triple_embeddings, -1)
        '''

        # multi all without masked_select
        tmp_triple_embeddings = one_all_embeddings.view(batch_size, 1, self.n_fnames, self.embedding_size).expand(batch_size, self.n_bi, self.n_fnames, self.embedding_size)
        tmp_bi_embeddings = bi_embeddings.view(batch_size, self.n_bi, 1, -1).expand_as(tmp_triple_embeddings)
        # B * N_BI * (N-2)
        tri_embeddings = torch.sum(tmp_bi_embeddings * tmp_triple_embeddings, -1)
        ...
        lr_out = torch.cat([bi_embeddings.view(batch_size, -1), all_embeddings.view(batch_size, -1), one_all_embeddings.view(batch_size, -1), tri_embeddings.view(batch_size, -1)], -1)
        for i in range(self.n_linear_layers):
            lr_in = lr_out
            if self.batch_norm:
                lr_in = self.batch_norms[i](lr_out)
