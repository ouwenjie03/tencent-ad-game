...
class DyNFFM(nn.Module):
    def __init__(self, fnames, max_idxs, embedding_size=4, dropout_rate=None, batch_norm=True, use_cuda=True):
        ...
        #  self.mask = make_mask(self.mask_len, self.n_fnames, fill_val=False)
        if self.use_cuda:
            self.mask = self.mask.cuda()
            self.fc_loss = self.fc_loss.cuda()

        # embedding lr layer, 3 layers
        #  self.embLr_input_dim = self.n_fnames * (self.n_fnames - 1) // 2 * self.embedding_size
        self.embLr_input_dim = self.n_bi * self.embedding_size
    ...
    def forward(self, static_ids, dynamic_ids, dynamic_lengths, conversion_rates):
        ...
        all_embeddings = all_embeddings.transpose(2, 1)
        all_embeddings_ll = torch.masked_select(all_embeddings, all_mask)
        ...
        embedding_lr_in_1 = bi_embeddings.view(batch_size, -1)
...
if __name__ == '__main__':
    ...
    dy_lengths = {"1":[3,2], "2":[1,5]}

    reals = [1, 0]

    dyNffm = DyNFFM([st_fnames, dy_fnames], [st_max_idxs, dy_max_idxs], use_cuda=True)
    dyNffm.cuda()
