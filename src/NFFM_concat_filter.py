...
class DyNFFM_concat_filter(nn.Module):
    def __init__(self, fnames, max_idxs, embedding_size=4, dropout_rate=None, batch_norm=True, use_cuda=True):
        ...
        super(DyNFFM_concat_filter, self).__init__()
        self.fnames = fnames
        ...
        for i in range(self.n_fnames):
            tmp = [0] * self.n_fnames
            tmp[i] = 1
            self.concat_mask.append(tmp)
        self.concat_mask = torch.autograd.Variable(torch.ByteTensor(self.concat_mask))

        # embedding lr layer, 3 layers
        # bi-interation + embedding concat
        self.embLr_input_dim = (self.n_bi + self.n_fnames) * self.embedding_size 
        ...
        self.concat_input_dim = self.hidden_size_3 + self.embLr_input_dim

        self.batch_norm_out = nn.BatchNorm1d(self.concat_input_dim)
        self.embLr_out = nn.Linear(self.concat_input_dim, 1)
        ...
        if self.use_cuda:
            ...
            self.concat_mask = self.concat_mask.cuda()
    ...
    def forward(self, static_ids, dynamic_ids, dynamic_lengths, conversion_rates):
        ...
        # one order
        all_concat_mask = self.concat_mask.view(1, self.n_fnames, self.n_fnames, 1).expand_as(all_embeddings)
        one_embeddings = torch.masked_select(all_embeddings, all_concat_mask)

        # embedding lr layer
        # B*F1*E + B*F2*E -> B*[F1+F2]*E -> B*[F*E]
        embedding_lr_in_1 = torch.cat([bi_embeddings.view(batch_size, -1), one_embeddings.view(batch_size, -1)], -1)
        ...
        # concat input and output
        embedding_lr_in = torch.cat([embedding_lr_in, embedding_lr_in_1], -1)
        if self.embLr_is_dropout:
